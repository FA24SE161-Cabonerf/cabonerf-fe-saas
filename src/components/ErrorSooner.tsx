type Props = {
	message: string;
};

export default function ErrorSooner({ message }: Props) {
	return (
		<div className="flex items-center space-x-3">
			{/* Icon */}
			<div className="h-fit w-[fit] rounded-xl bg-[#d95a3b] p-1.5 shadow-sm shadow-[#fee2e2]">
				<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width={21} height={21} color={'#fff'} fill={'#fff'}>
					<path
						d="M5.32171 9.6829C7.73539 5.41196 8.94222 3.27648 10.5983 2.72678C11.5093 2.42437 12.4907 2.42437 13.4017 2.72678C15.0578 3.27648 16.2646 5.41196 18.6783 9.6829C21.092 13.9538 22.2988 16.0893 21.9368 17.8293C21.7376 18.7866 21.2469 19.6548 20.535 20.3097C19.241 21.5 16.8274 21.5 12 21.5C7.17265 21.5 4.75897 21.5 3.46496 20.3097C2.75308 19.6548 2.26239 18.7866 2.06322 17.8293C1.70119 16.0893 2.90803 13.9538 5.32171 9.6829Z"
						stroke="#fff"
						strokeWidth="1.5"
					/>
					<path d="M11.992 16H12.001" stroke="#d95a3b" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
					<path d="M12 13L12 8.99997" stroke="#d95a3b" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
				</svg>
			</div>
			{/* Description */}
			<div>
				<div className="font-medium">{message}</div>
			</div>
		</div>
	);
}
