...
export function toSlottedItems<Item>(items: Array<Item>, idField: keyof Item, slotItemMap: SlotItemMapArray): SlottedItems<Item> {
	return slotItemMap.map((slotItem) => ({
		slotId: slotItem.slot,
		itemId: slotItem.item,
		item: slotItem.item === '' ? null : items.find((item) => slotItem.item === item[idField])!,
	}));
}

export function initSlotItemMap<Item>(items: Array<Item>, idField: keyof Item): SlotItemMapArray {
...
export function dynamicSwapy<Item>(
	...
	const newItems: SlotItemMapArray = items
		.filter((item) => !slotItemMap.some((slotItem) => slotItem.item === item[idField]))
		.map((item) => ({
			slot: item[idField] as string,
			item: item[idField] as string,
		}));
	...
	if (!removeItemOnly) {
		withoutRemovedItems = slotItemMap.filter((slotItem) => items.some((item) => item[idField] === slotItem.item) || !slotItem.item);
	} else {
