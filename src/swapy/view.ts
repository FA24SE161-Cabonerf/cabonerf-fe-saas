...
export interface View {
	...
	layoutRect(): Rect;
	boundingRect(): Rect;
	usePlugin<P extends ViewPlugin, C>(pluginFactory: (v: View, config: C) => P, config: C): P;
...
export function createView(el: HTMLElement): View {
	...
	const borderRadius = parseBorderRadius(window.getComputedStyle(element).borderRadius);
	const thisView = {
		el: () => element,
		setTransform,
		clearTransform,
		...
		boundingRect: () => createRectFromBoundingRect(element.getBoundingClientRect()),
		usePlugin,
		destroy,
		updateElement,
	};
	...
	function renderTransform() {
		const { dragX, dragY, translateX, translateY, scaleX, scaleY } = currentTransform;
		if (dragX === 0 && dragY === 0 && translateX === 0 && translateY === 0 && scaleX === 1 && scaleY === 1) {
			element.style.transform = '';
		} else {
			element.style.transform = `translate(${dragX + translateX}px, ${dragY + translateY}px) scale(${scaleX}, ${scaleY})`;
		}
	}

	function usePlugin<P extends ViewPlugin, C>(pluginFactory: (v: View, config: C) => P, config: C) {
